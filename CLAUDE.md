...
이 저장소에는 현재 **기획 문서만 존재**하며, 애플리케이션 코드나 패키지 매니페스트, 스캐폴딩은 아직 없습니다. 코드를 작성하기 전에 아래 "기술 스택" 항목에 따라 Next.js 프로젝트를 초기화해야 합니다.

## 이 프로젝트는 무엇인가

"Daily Economy Top 10 Topics" — 하루 한 번, 연합인포맥스(Yonhap Infomax) RSS 피드에서 경제 뉴스 제목을 수집하고, 한국어 형태소 분석으로 그날 가장 많이 등장한 키워드 Top 10을 추출한 뒤, 사전 기반 매칭으로 각 키워드의 감성 점수를 보여주는 단일 페이지 Next.js 앱입니다. 로그인 없음, 데이터 영구 저장 없음, 감성 분석 과정에 LLM을 쓰지 않습니다.
...
- **백엔드**: Python 서버리스 함수 2개, Vercel Python 런타임에 배포 — Node/Next.js API 라우트가 아닙니다
  - `/api/keywords.py` — 뉴스 키워드·감성 분석
  - `/api/markets.py` — 시장 지표(지수·금리·환율), keywords.py와 무관한 별도 함수
...
- **외부 API 키 불필요** — RSS 피드, 두 감성사전, `yfinance` 모두 공개 자원이거나 인증이 필요 없는 라이브러리입니다. `.env`에는 현재 앱 런타임과 무관한 도구용 자격증명(GitHub/Supabase/Vercel/OpenAI)이 들어 있으며, 이 파일은 계속 gitignore 상태로 유지되어야 합니다.

## 이 저장소의 데이터 소스

- [`econNLPdic.txt`](econNLPdic.txt) — 재무 특화 감성사전 60개 항목(KOSELF, 조수지·김흥규·양철원 2021). 79줄뿐이라 직접 읽어도 무방합니다.
- `SentiWord_info.json` (KnuSentiLex, 약 14,843개 항목, 약 15,000줄) — **아직 저장소에 없으며 추가되어야 합니다.** 추가된 이후에도 전체를 직접 열어 읽지 말고 코드로만 로드하세요 — 불필요한 토큰 낭비를 유발합니다.
...
- econNLPdic의 일부 단어(이상/매우/진행/수치/결국/전환/제외/매각/처리/공격/바닥)는 PRD.md 5장에서 실제 순위를 확인한 뒤 제외 여부를 결정하기로 한 후보로 표시되어 있습니다. 구현 전에 현재 상태를 확인하세요.

## 명시적으로 범위 밖인 것들

로그인/회원가입 없음, 기사 본문 열람 화면 없음(키워드별 기사 **목록**은 있음 — 키워드를 누르면 제목 + 원문 링크가 펼쳐짐), 경제 외 카테고리 없음, 과거 날짜·추이 조회 없음(키워드 기준. 시장 지표는 최근 1개월 그래프를 보여줌 — PRD.md M7 참고), 외부 DB나 영구 저장 없음, LLM 기반 요약·감성 판정 없음(사전 규칙만 사용), 기사 본문 크롤링 없음(제목만 사용).
