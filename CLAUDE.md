...
- [`noise_topics.txt`](noise_topics.txt) — 제목에 포함되면 **기사 전체**를 집계에서 제외하는 정치·사법 절차 용어 목록(수사권/탄핵/국회 등). "정책/금융" RSS 피드 자체는 이미 제외했고(PRD §5 M0), 이건 남은 5개 피드에 섞여 들어올 수 있는 정치 기사에 대한 이중 안전장치다. stopwords.txt와 다른 층위이니 혼동하지 말 것.
